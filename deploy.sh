...
export FISSION_ENVIRONMENT_NAME="env-get-term"
export FISSION_FUNCTION_NAME="fn-get-term"
export FISSION_FUNCTION_ROUTE="/term"
export FISSION_FUNCTION_ROUTE_NAME="route-get-term"
...
echo "- Creating function: $FISSION_FUNCTION_NAME..."; fission fn create --spec --name $FISSION_FUNCTION_NAME --env $FISSION_ENVIRONMENT_NAME --src "./func/*" --entrypoint main.get_term --executortype newdeploy --maxscale 1 || { echo "ERROR: Failed to create function. Message: Make sure the script is run only once, or run the remove_fission.sh script. [FINISHING SCRIPT]"; exit; }
